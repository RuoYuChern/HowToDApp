...
因此从这里我们可以总结出什么是智能合约：智能合约是一个具有地址（账户号）、余额、状态、代码的账户，称为合约账户。常规的用户账户（EOA）只有：地址、余额。常规用户账户是被用户控制，合约账户是不被用户控制，因为用户掌握不了合约账户的私钥。合约账户是被合约本身拥有，最终**被以太坊EVM控制**。

<figure><img src="../.gitbook/assets/zhinengheyue.png" alt=""><figcaption></figcaption></figure>
